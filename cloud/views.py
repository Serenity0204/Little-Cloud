...
from .forms import FileUploadForm, ShareFileForm
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            exists = File.objects.filter(
                title=file_instance.title, user=request.user
            ).exists()
            if exists:
                messages.error(request, "File with this Title already exists")
...
@login_required(login_url="login")
def share_others_view(request):
    if request.method == "POST":
        form = ShareFileForm(request.user, request.POST)
        ...
        if form.is_valid():
            ...
            if user is None:
                ...
                return redirect("share_others")
            # check if it's self
            if user == request.user:
                messages.error(request, "Sharing to yourself is not allowed")
                return redirect("share_others")
        ...
        form = ShareFileForm(request.user)
